...
public class HubExceptionHandler {
    ...
    public ResponseEntity HubNotFoundException(HubException.HubNotFoundException e) {
        log.info(e.getMessage());
        return ResponseEntity.badRequest().body(SingleResponse.error(e.getMessage(), "HUB_ERROR_3"));
    }
    ...
    public ResponseEntity HubAlreadyDeletedException(HubException.HubAlreadyDeletedException e) {
        log.info(e.getMessage());
        return ResponseEntity.badRequest().body(SingleResponse.error(e.getMessage(), "HUB_ERROR_4"));
    }

    @ExceptionHandler(HubException.HubDeniedException.class)
    public ResponseEntity HubDeniedException(HubException.HubDeniedException e) {
        log.info(e.getMessage());
        return ResponseEntity.badRequest().body(SingleResponse.error(e.getMessage(), "HUB_ERROR_5"));
    }
}
